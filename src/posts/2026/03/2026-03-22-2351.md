...
Web版VSCodeの環境によっては、`git-commit` 用のスニペットがうまくフックされない場合：

* **手順:** `Configure User Snippets` で `git-commit` ではなく **`Global Snippets` (New Global Snippets file)** を作成し、名前を `my-git.code-snippets` にして保存する。
* **理由:** グローバル設定にすることで、どの入力欄でも `post` というキーワードに反応するようになります。
